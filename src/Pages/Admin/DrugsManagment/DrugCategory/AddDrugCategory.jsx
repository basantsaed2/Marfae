...
const AddDrugCategory = ({ lang = 'en' }) => {
    const apiUrl = import.meta.env.VITE_API_BASE_URL;
    const { postData, loadingPost, response: postResponse } = usePost({ url: `${apiUrl}/admin/addDrugCategory` });
    ...
    const title = isEditMode ? 'Edit Drug Category' : 'Add Drug Category';

    // Define the fields for the form
    const fields = [
        { name: 'drugCategory', type: 'input', placeholder: 'Drug Category Name' },
        { name: 'description', type: 'input', placeholder: 'Drug Category Description' },
        {
            type: "switch",
            name: "status",
            placeholder: "Status",
    ...
    useEffect(() => {
        if (initialItemData) {
            ...
            setValues({
                ...
                drugCategory: initialItemData.drugCategory || '',
    ...
    const handleSubmit = async () => {
        if (isEditMode) {
            ...
            const data = {
                id: values.id || "",
                name: values.drugCategory || "",
                description: values.description || "",
                status: values.status || "inactive",
            };
            await changeState(
                `${apiUrl}/admin/editDrugCategory/${values.id}`,
            ...
            body.append("name", values.drugCategory || "");
            body.append("description", values.description || "");
            body.append("status", values.status || "inactive");
            await postData(body, "Drug Category Added Successfully!");
        }
    };
