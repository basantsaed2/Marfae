...
const AddCountry = ({ lang = 'en' }) => {
    const apiUrl = import.meta.env.VITE_API_BASE_URL;
    const { postData, loadingPost, response: postResponse } = usePost({ url: `${apiUrl}/admin/addCountry` });
    ...
    const title = isEditMode ? 'Edit Country' : 'Add Country';

    // Define the fields for the form
    const fields = [
        { name: 'country', type: 'input', placeholder: 'Country Name' },
        {
            type: "switch",
            name: "status",
            placeholder: "Status",
    ...
    useEffect(() => {
        if (initialItemData) {
            ...
            setValues({
                ...
                country: initialItemData.country || '',
    ...
    const handleSubmit = async () => {
        if (isEditMode) {
            ...
            await changeState(
                `${apiUrl}/admin/editCountry/${values.id}`,
            ...
            body.append("name", values.country || "");
            body.append("status", values.status);
            await postData(body, "Country Added Successfully!");
        }
    };
