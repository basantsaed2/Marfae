import React, { useState, useEffect } from "react";
import { Table} from '@/components/ui/table';
import { Plus } from 'lucide-react';
import { Link, useNavigate } from 'react-router-dom';
import { useGet } from '@/Hooks/UseGet';
import { useDelete } from '@/Hooks/useDelete';
import DeleteDialog from '@/components/DeleteDialog'; // Import DeleteDialog
import FullPageLoader from "@/components/Loading";

const JobSpecialization = () => {
    const apiUrl = import.meta.env.VITE_API_BASE_URL;
    const { refetch: refetchSpecialization, loading: loadingSpecialization, data: dataSpecialization } = useGet({ url: `${apiUrl}/admin/getSpecializations` });
    const { deleteData, loadingDelete } = useDelete();
    const [isDeleteOpen, setIsDeleteOpen] = useState(false);
    const [selectedRow, setSelectedRow] = useState(null);

    const [specializations, setSpecializations] = useState([]);
    const navigate = useNavigate();

    useEffect(() => {
        refetchSpecialization();
    }, [refetchSpecialization]);

    useEffect(() => {
        if (dataSpecialization && dataSpecialization.specializations) {
            const formatted = dataSpecialization?.specializations?.map((u) => ({
                id: u.id,
                specialization: u.name || "—",
                status: u.status === "active" ? "Active" : "Inactive",
            }));
            setSpecializations(formatted);
        }
    }, [dataSpecialization]);

    const Columns = [
        { key: "specialization", label: "Job Specialization" },
        { key: "status", label: "Status" },
    ];

    const handleEdit = (item) => navigate(`add`, { state: { itemData: item } });

    const handleDelete = (item) => {
        setSelectedRow(item);
        setIsDeleteOpen(true);
    };

    const handleDeleteConfirm = async () => {
        if (!selectedRow) return;

        const success = await deleteData(
            `${apiUrl}/admin/deleteSpecialization/${selectedRow.id}`,
            `${selectedRow.specialization} Deleted Successfully.`
        );
        if (success) {
            setIsDeleteOpen(false);
            setSpecializations((prev) => prev.filter((item) => item.id !== selectedRow.id));
            setSelectedRow(null);
        }

    };

    return (
        <div className="p-4">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl text-bg-primary font-bold">Specializations</h2>
                <Link
                    to="add"
                    className="flex justify-center items-center px-4 py-1 rounded-md text-base bg-bg-secondary font-semibold text-white hover:bg-bg-secondary/90"
                >
                    <Plus className="mr-2 h-4 w-4 text-white" /> Add Specialization
                </Link>
            </div>
            {loadingSpecialization ? (
                <FullPageLoader />
            ) : (
                <Table
                    data={specializations}
                    columns={Columns}
                    statusKey="status"
                    filterKeys={["specialization","status"]}
                    titles={{ specialization: "Specialization" }}
                    onEdit={(item) => handleEdit({ ...item, type: 'specialization' })}
                    onDelete={handleDelete}
                    className="w-full bg-white rounded-lg shadow-md p-6"
                />
            )}
            <DeleteDialog
                open={isDeleteOpen}
                onOpenChange={setIsDeleteOpen}
                onDelete={handleDeleteConfirm}
                name={selectedRow?.specialization}
                isLoading={loadingDelete}
            />
        </div>
    );
};

export default JobSpecialization;