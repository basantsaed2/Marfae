...
const AddQualifications = ({ lang = 'en' }) => {
    const apiUrl = import.meta.env.VITE_API_BASE_URL;
    const { postData, loadingPost, response: postResponse } = usePost({ url: `${apiUrl}/admin/add-job-qualification` });
    ...
    const title = isEditMode ? 'Edit Qualification' : 'Add Qualification';

    // Define the fields for the form
    const fields = [
        { name: 'qualification', type: 'input', placeholder: 'Qualification Name' },
        {
            type: "switch",
            name: "status",
            placeholder: "Status",
    ...
    useEffect(() => {
        if (initialItemData) {
            ...
            setValues({
                ...
                qualification: initialItemData.qualification || '',
    ...
    const handleSubmit = async () => {
        if (isEditMode) {
            ...
            await changeState(
                `${apiUrl}/admin/edit-job-qualification/${values.id}`,
            ...
            body.append("name", values.qualification || "");
            body.append("status", values.status || "inactive");
            await postData(body, "Qualification Added Successfully!");
        }
    };
